...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get paginated results with statistics including fix information
    ...
    const result = await pool.query(
      `SELECT 
        ...
        COUNT(CASE WHEN cr.status = 'attention' AND f.id IS NULL THEN 1 END) as attention,
        COUNT(CASE WHEN cr.status = 'attention' AND f.id IS NOT NULL THEN 1 END) as attention_fixed,
      ...
      LEFT JOIN fixes f ON f.previous_check_result_id = cr.id AND f.account_id = $1
      WHERE td.account_id = $1 ${versionFilter}
    ...
    return NextResponse.json({
      threadlines: result.rows.map(row => ({
        ...
        results: {
          ...
          attentionFixed: parseInt(row.attention_fixed) || 0,
